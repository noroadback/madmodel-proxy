...
把清华 madmodel 服务变成一个**本机常驻、即插即用的 OpenAI 兼容端点**。
...
madmodel（`madmodel.cs.tsinghua.edu.cn`，清华大学高性能计算中心部署的 DeepSeek 服务）**本身就提供 OpenAI 格式的 API**（官方文档甚至给了 `openai` 库的直连示例）——所以本项目做的不是"协议转译"。真正拦在"标准 OpenAI 客户端直接可用"面前的是两道鸿沟，本项目存在的全部理由就是填平它们：

1. **时间**：API token 只有 5 小时有效期，且官方获取方式是"网页登录 → 手动复制"→ 本项目的续期守护用 DPAPI 存储的凭据自动走完整统一认证链（可信设备登记后续期免二次认证），到期前 30 分钟自动换新，代理按文件热加载，客户端零感知、永不需要碰 token；
2. **行为**：上游虽是 OpenAI 格式，行为却与客户端预期处处相悖（2026-09-06 直连实测逐条复现，见"已验证的上游行为"）：非流式请求 60 秒整被网关掐断、WAF 误拦含工具语法的明文请求体、一切错误包装成 `HTTP 200 + "服务器繁忙"`、`GET /v1/models` 返回网页 HTML、`logprobs` 等参数被拒、流可能截断而无 `[DONE]` → 代理逐项适配：对上游恒以流式请求+为非流式客户端聚合、请求体 gzip 编码、错误翻译为真实状态码与原因、伪造 models 端点、参数剥离、截断检测不伪造成功；
3. **位置**：校园服务 → 本机 `127.0.0.1:8080` 常驻端点（本地 Bearer 鉴权，学校 token 不进任何客户端配置，对话数据流经你可控的进程）。

一句话：**上游"格式兼容"但"行为不兼容"且"凭据短命"——本项目让它对任意 OpenAI 客户端变成一个配好就不用管的本地端点。**
...
**能接入哪些客户端**：任何支持"自定义 OpenAI Base URL"的智能体/客户端都能接入——指向 `http://127.0.0.1:8080/v1`、用本地 key 鉴权即可，协议层就是标准 OpenAI 形态（chat completions + models）。**边界**要清楚：只有 chat completions 一个业务端点（无 embeddings/图像/音频等）；单模型（任意模型名都会被重写为 `DeepSeek-V4-Flash`，见"设计取舍"）；`logprobs`/`n>1` 被剥离（上游拒绝）；代理宿主机必须是 Windows,客户端本身不限平台。dsh 之外(codex、claude code 等)**未逐一验证**——欢迎在 issue 中附上你的客户端与结果。
...
上游 madmodel 前置的 WAF 存在**误报**:其 SQL 注入特征规则会拦截请求体明文中包含字面量 `"(set "` 的请求(该字面量常见于 agent 工具的系统提示词,如 dsh 的 `"(set wait: true)"` 工具语法)——对 JSON 对话体而言这是误判。2026-09-06 直连实测复现:同一请求体,明文发送 → 404 HTML 拦截页;gzip 编码发送 → 正常通过。拦截发生在鉴权之前,与 token 无关。
...
- **一切错误包装在 `HTTP 200` 里**(2026-09-06 直连实测):非流式形态为 `200 + {"status":10001/10003,"message":"服务器繁忙/您无此权限"}`——包括认证失败、模型不存在、参数被拒这些本该是 401/404/400 的情形;流式形态为 SSE 内嵌 `{"errorMessage":"..."}`。直连客户端会把它们误判为上游过载而盲目重试,或当成成功空响应。代理翻译为真实 4xx/5xx 与原因。
