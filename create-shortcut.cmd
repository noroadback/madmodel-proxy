...
powershell -NoProfile -Command "$ws = New-Object -ComObject WScript.Shell; $lnk = $ws.CreateShortcut([IO.Path]::Combine([Environment]::GetFolderPath('Desktop'), 'madmodel.lnk')); $lnk.TargetPath = '%~f0\..\start.cmd'; $lnk.WorkingDirectory = '%~dp0.'; $lnk.WindowStyle = 7; $lnk.Description = 'madmodel proxy'; $lnk.Save(); exit 0" >nul 2>&1
if %errorlevel%==0 (
