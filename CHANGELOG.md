...
### 移除:本地 Bearer 鉴权(2026-09-07)

- 删除本地鉴权层与 `key` 命令、`PROXY_API_KEY` / `PROXY_NO_AUTH` 配置、`api-key` 状态文件。安全边界收窄为**本机回环(只监听 127.0.0.1)+ Host 白名单**;客户端 API key 随便填(仅为满足界面非空校验)。
- 取舍依据:单用户个人机器上,鉴权挡的"本机恶意网页盲调配额"是低频威胁,而 key 的获取与配置是真实的新用户摩擦(冷启动验收实证)。需要鉴权的用户可在 `adapters/http-server.js` 自行加回(Bearer 解析约 20 行)。
- 顺带修复(冷启动验收发现,均已带回归测试):
  - 管道(非 TTY)login 永久挂死——`rl.close()` 在管道模式会结束 stdin,密码监听永等;
  - TTY login 密码明文回显——上一修复把 close 挪后导致 readline echo 与 `*` 掩码双写。
