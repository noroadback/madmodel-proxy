...
- 测试计数口径（与 README 一致）。`test-auth` 38，`node --test` 单元 70（含 scheduler 10、inflight 8），`test-proxy` 端到端 69，合计 177 项；`test-creds` 的 3 项 DPAPI 自检不计入。
