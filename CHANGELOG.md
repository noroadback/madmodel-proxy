...
- 上游移除了 madmodel 端点的 WAF（经工单向学校信息服务反馈后的正式处理，thu-info 社区在 issue 中确认，本仓库实测复核：非压缩请求体携带工具语法与 `(set ` 触发字面量直发通过）。此前 WAF 的 SQL 注入特征规则会误拦含该字面量的明文请求体，代理以 gzip 编码规避——规避随之移除，`PROXY_NO_GZIP` 配置一并删除，请求体恢复明文直发。
