// core/scheduler.js
// 续期调度状态机(纯编排,不含认证协议细节——不知道登录页面、Cookie、SM2
// 或验证码;认证通过注入的 refresh() 完成)。
//
// 状态全集:WAITING(等待续期窗口)/ REFRESHING(认证中)/ BACKOFF(失败退避)/
// STOPPED(退出)。
// 规则:
...
//   - 失败按指数退避;文件事件可提前唤醒状态检查,但不能绕过退避截止时间
...
class Scheduler {
  ...
  async run() {
    ...
    let nextAttemptAt = 0;
    let inBackoff = false; // 闸门等待期间的呈现状态:失败退避=BACKOFF,成功防忙循环=WAITING
    ...
    try {
      for (;;) {
        ...
        if (this.now() < nextAttemptAt) {
          this.state = inBackoff ? STATE.BACKOFF : STATE.WAITING;
        ...
        try {
          ...
          if (e.code === 'TWO_FACTOR_REQUIRED') { this.state = STATE.STOPPED; throw e; }
          badCredsStreak = e.code === 'BAD_CREDENTIALS' ? badCredsStreak + 1 : 0;
          if (badCredsStreak >= 3) { this.state = STATE.STOPPED; throw e; }
...
module.exports = { Scheduler, STATE };
